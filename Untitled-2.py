...
GUILD_ID = int(os.getenv("GUILD_ID", "1477686598647808130"))
SPREADSHEET_NAME = os.getenv("SPREADSHEET_NAME", "TDC 연락망 무단탈주인원 명단의 사본")
...
def create_sheet() -> str:
    global sheet
    new_title = get_today_sheet_name()

    try:
        try:
            sheet = spreadsheet.worksheet(new_title)
            return f"이미 존재하는 시트로 전환됨: {new_title}"
        except WorksheetNotFound:
            pass

        template_sheet = spreadsheet.get_worksheet(0)
        new_sheet = template_sheet.duplicate(new_sheet_name=new_title)
...
def ensure_sheet_rows(target_row: int) -> None:
    ...
    if target_row > current_rows:
        sheet.add_rows(target_row - current_rows)
...
async def is_kicked_or_banned(member: discord.Member) -> bool:
    ...
    async for entry in member.guild.audit_logs(limit=10, action=discord.AuditLogAction.kick):
        if entry.target and entry.target.id == member.id and (now - entry.created_at).total_seconds() < 10:
            return True
...
if not TOKEN:
    raise ValueError("DISCORD_BOT_TOKEN이 .env에 설정되지 않았습니다.")
